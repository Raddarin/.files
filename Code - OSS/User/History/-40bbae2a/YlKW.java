...
public class Mountain extends Fractal {
	...
	public void draw(TurtleGraphics turtle) {
		...
		fractalMount(turtle, order,2, 0, p1, p1, p2, p3);
	
	}

    private Point midPoint (Point p1, Point p2) {
        int mx = (p1.getX() + p2.getX()) / 2;
        int my = 20 + (p1.getY() + p2.getY()) / 2;
    ...
    private void fractalMount(TurtleGraphics turtle, int order, int divader, int reverse, Point start_point, Point a_Point, Point b_Point, Point c_Point) {
        ...
        if (order == 0) {
            ...
            */
			...
			fractalMount(turtle, order-1, divader*2, reverse,  start_point, a_Point, midPoint(a_Point, b_Point), midPoint(a_Point, c_Point));

            
            fractalMount(turtle, order-1, divader*2, reverse, midPoint(a_Point, b_Point), midPoint(a_Point, b_Point), b_Point, midPoint(b_Point, c_Point));


            fractalMount(turtle, order-1, divader*2, reverse+ 180,  midPoint(b_Point, c_Point), midPoint(b_Point, c_Point), midPoint(a_Point, b_Point), midPoint(a_Point, c_Point));


            fractalMount(turtle, order-1, divader*2, reverse, midPoint(a_Point, c_Point),  midPoint(a_Point, c_Point), midPoint(b_Point, c_Point), c_Point);
